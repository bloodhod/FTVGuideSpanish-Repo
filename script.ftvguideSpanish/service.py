...
try:
    ADDON = xbmcaddon.Addon('script.ftvguideSpanish')
    if ADDON.getSetting('cache.data.on.xbmc.startup') == 'true':
        Service()
    if ADDON.getSetting('autostart') == "true":
        xbmc.executebuiltin("RunAddon(script.ftvguideSpanish)")
except source.SourceNotConfiguredException:
    pass  # ignore
except Exception, ex:
    xbmc.log('[script.ftvguideSpanish] Uncaught exception in service.py: %s' % str(ex) , xbmc.LOGDEBUG)